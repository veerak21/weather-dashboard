...
function futureCondition(lat, lon) {
    ...
    var futureURL = `http://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&units=metric&exclude=current,minutely,hourly,alerts&appid=${apiKey}`;
    ...
    $.ajax({
        ...
        for (let i = 1; i < 6; i++) {
            ...
            var futureCard = $(`
                <div class="pl-3">
                    <div class="card pl-4 pt-2 mb-2 w-20px bg-primary text-white">
                        <div class="card-body">
                            <h5>${currDate}</h5>
                            <p>${iconURL}</p>
...
$("#search-btn").on("click", function(event) {
    ...
    if (!searchHistoryList.includes(city)) {
        ...
        var searchedCity = $(`
            <li class="list-group-item m-1">${city}</li>
            `);
